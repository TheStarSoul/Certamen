...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Error encontrado, no se encuentran los adapter para los spinner
        ArrayAdapter<String> adCategoria = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, CategoriaS);
        Categoria.setAdapter(adCategoria);

        ArrayAdapter<String> adUbicacion = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, UbicacionS);
        Ubicacion.setAdapter(adUbicacion);
    ...
    public void BuscarProducto(View view){
        //Error encontrado en el AdminSQLiteOpenHelper, el name esta con guion bajo _, lo quite
        AdminSQLiteOpenHelper admin = new AdminSQLiteOpenHelper(this, "ControlInventario", null, 1);
        SQLiteDatabase BaseDatos = admin.getWritableDatabase();
        String idProducto = ID.getText().toString();
    ...
    public void EliminarProducto(View view){
        ...
        if (!IDProducto.isEmpty()){
            ...
            if (Eliminar == 1){
                //Error encontrado, en la validacion de la consulta de eliminar, no hay codigo
                ID.setText("");
                Nombre.setText("");
                Categoria.setSelection(0);
                Cantidad.setText("");
                ...
                Toast.makeText(this, "La eliminacion de los datos se realizo correctamente", Toast.LENGTH_LONG).show();
                BaseDatos.close();
            } else {
    ...
    public void ModificarProducto(View view){
        ...
        String NombreProducto = Nombre.getText().toString();
        String CategoriaS = Categoria.getSelectedItem().toString();
        //Error en el modificar al instancear directamente en Integer tanto cantidad y precio, los cambie a String
        String cantidad = Cantidad.getText().toString();
        String precio = Precio.getText().toString();
        String ubicacion = Ubicacion.getSelectedItem().toString();
        if (!IDproducto.isEmpty() && !NombreProducto.isEmpty() && !CategoriaS.equals("Seleccione:") && !cantidad.equals("") && !precio.equals("") && !ubicacion.equals("Seleccione:")){
            ...
            if (Update == 1){
                //Error encontrado, en la validacion del update no se limpia los campos al realizar la modificacion, a su  vez que no se informa al usuario si se realizo correctamente el update o no
                ID.setText("");
                Nombre.setText("");
                Categoria.setSelection(0);
                Cantidad.setText("");
                ...
                Toast.makeText(this, "La actualización de datos se realizo correctamente", Toast.LENGTH_LONG).show();
                BaseDatos.close();
            }
        } else {
